...
export const drawHands = (hands, ctx, showNames = false) => {
	...
	for (let i = 0; i < hands.length; i++) {
		...
		const isLeftHand = hands[i].handedness.toLowerCase() === 'left';
		const bottomY = canvasHeight - bottomMargin;

		if (isLeftHand) {
			// Left hand text on the right side (will be flipped to appear on left)
			ctx.translate(canvasWidth, 0);
			ctx.scale(-1, 1);
			ctx.textAlign = 'left';
			ctx.fillText(
				`Left Pinching: ${isPinching ? 'Yes' : 'No'}`,
				sidePadding,
				bottomY - lineHeight
			);
			ctx.fillText(
				`Left Threshold: ${adaptivePinchThreshold.toFixed(2)}`,
				sidePadding,
				bottomY
			);
		} else {
			// Right hand text on the left side (will be flipped to appear on right)
			ctx.translate(canvasWidth, 0);
			ctx.scale(-1, 1);
			ctx.textAlign = 'right';
			ctx.fillText(
				`Right Pinching: ${isPinching ? 'Yes' : 'No'}`,
				canvasWidth - sidePadding,
				bottomY - lineHeight
			);
			ctx.fillText(
				`Right Threshold: ${adaptivePinchThreshold.toFixed(2)}`,
				canvasWidth - sidePadding,
				bottomY
			);
		}
