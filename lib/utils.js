...
const estimateHandSize = (hand) => {
	...
	return calculateDistance(wrist, middleFinger);
};
...
export const drawHands = (hands, ctx, showNames = false) => {
	...
	const basePinchThreshold = 30; // Adjust this value to set the pinch sensitivity
	...
	for (let i = 0; i < hands.length; i++) {
		...
		const averageHandSize = 150; // You may need to adjust this based on your specific setup
		const scaleFactor = handSize / averageHandSize;
		...
		ctx.fillStyle = 'white';
		ctx.font = '16px Arial';
		ctx.fillText(`Pinching: ${isPinching ? 'Yes' : 'No'}`, 10, 30 + i * 60);
		ctx.fillText(
			`Threshold: ${adaptivePinchThreshold.toFixed(2)}`,
			10,
			50 + i * 60
		);
	}
